...
def main() -> int:
    ap = argparse.ArgumentParser(description="LC-01 RadarSimPublic constant-acceleration algorithm adapter")
    ...
    # Upstream CA helper uses [x, y, vx, vy, ax, ay].  The frozen E2 cases
    # contain no acceleration, so the new algorithm begins with zero acceleration.
    kf.x[:] = np.array(
        [initial_position[0], initial_position[1], true_velocity[0], true_velocity[1], 0.0, 0.0],
        dtype=float,
    )
    ...
    evidence = {
        "adapter": "RadarSimPublicConstantAccelerationTMSUAdapter",
        "algorithm_profile_id": "radarsimpublic.constant_acceleration_kf.v1",
        ...
        "selected_tracking_algorithm": "initialize_constant_acceleration_filter",
        "state_layout": "[x, y, vx, vy, ax, ay]",
        "radar_parameters": {
        ...
        "algorithm_change_is_substantive": True,
        "note": "LC-01 deliberately changes the selected upstream tracking algorithm from constant-velocity KF to constant-acceleration KF while keeping the capability contract and semantic profile fixed."
    }
