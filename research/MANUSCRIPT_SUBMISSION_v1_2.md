...
**Author-final submission candidate v1.2 — WP1 evidence locked**

## Abstract

**Purpose:** Digital test and evaluation (T&E) requires both a stable method for unifying heterogeneous simulation models and a defensible rule for qualification evidence as those models evolve. We evaluate a Simulation Abstraction Layer (SAL)-aligned Test Model Service Unit (TMSU) that separates trial-facing capability identity from concrete implementation identity.

**Methods:** Using OpenEaagles TWS/AirTrkMgr and an independent RadarSimPublic radar/tracking implementation, we tested behavior preservation, binding-only substitution, semantic precheck, intended-use qualification, cumulative evidence, controlled version carry-forward, and an algorithm-change stopping rule.

**Results:** OpenEaagles wrapper behavior was preserved in 16/16 cases, and both implementations executed 16/16 cases behind one frozen upper trial without trial edits. Semantic precheck rejected 5/5 injected mismatches while retaining a real RF relation as `UNKNOWN`. A strict byte-level carry-forward rule failed (8/16 exact traces), whereas an evidence-type-aware numerical rule supported a controlled provenance revision (16/16 normalized equivalence). Changing the tracker from constant-velocity to constant-acceleration prevented automatic inheritance of implementation-specific qualification despite both configurations executing all legacy cases.

**Conclusions:** TMSU provides bounded heterogeneous-model unification, but test-grade reuse also requires versioned, dependency-aware evidence lifecycle management. Evidence history can accumulate monotonically while current qualification remains configuration-, intended-use-, and evidence-dependent.
...
A capability identifies what the trial requests; an implementation identifies what software/model instance generated evidence. TMSU associates a capability contract, semantic declarations, executable binding, provenance, and evidence with that boundary.

This study makes two linked contributions. First, it proposes and empirically instantiates a method for **heterogeneous model unification** that preserves implementation heterogeneity while stabilizing upper-trial logic. Second, it extends that organization into a **configuration-aware evidence lifecycle** that distinguishes historical retention, current applicability, delta reassessment, and refusal of automatic inheritance.
...
where `I` is implementation, `U` intended use, `C` current configuration, and `E_t` the accumulated evidence available at time `t`. `QUALIFIED_WITHIN_EVIDENCE` denotes support for a declared bounded research use only; it is not authoritative accreditation.
...
Otherwise the record is retained historically but becomes stale for the changed configuration until appropriate affected evidence is supplied. `UNKNOWN` denotes evidence insufficiency and is not treated as implicit compatibility or invalidity.
...
MS-01 introduced the independent public `Murmur-ops/RadarSimPublic` implementation at frozen commit `8b63f824a5744c1b3a3fca5e948fa7c59f897b17`. Its adapter invoked upstream radar/SNR, Kalman-filter, and track-quality components without upstream source changes. Upper trial, orchestrator, capability contract, and semantic-profile identifier were frozen. Substitution changed implementation binding only. MS-01 tested architectural/contract substitutability, not equal behavior or fidelity.
...
Within the tested scope, RQ1 was supported: a real legacy implementation and an independent heterogeneous implementation were organized behind a stable trial-facing capability boundary without upper-trial rewriting. The result establishes architectural/contract-level unification, not model equivalence.
...
### 4.4 RQ4 — Inheritance required both a carry-forward rule and a stopping rule
...
Automatic implementation-specific qualification inheritance was therefore rejected. Unaffected BP-01 and SP-01 evidence remained active. Prior CV-specific MS/EQ/EB/VU evidence remained historically retained but stale for the CA configuration. Fresh CA architectural execution was established as `PASS_FRESH_EXECUTION`, while CA kinematic intended-use fitness remained `UNKNOWN_PENDING_FRESH_FITNESS_EVIDENCE`; the RF states also remained `UNKNOWN`.

Within the tested scope, RQ4 was supported: the lifecycle permitted carry-forward for one controlled provenance revision and enforced a stopping rule for a substantive algorithm/implementation-identity change.
...
This distinction is important for modernization of strongly coupled legacy software: the integration boundary can be standardized while valuable internal model logic remains intact. TMSU is consequently complementary to, rather than a replacement for, existing runtime and exchange mechanisms.
...
The persistent RF `UNKNOWN` demonstrates the value of a non-binary state. The mapping is structurally legal and executable, but current evidence does not establish the physical concept relation required for the RF-performance use. The lifecycle therefore preserves the uncertainty state rather than converting absence of contradiction into trust.
...
### 5.4 Bounded inheritance needs both a carry-forward rule and a refusal rule
...
The research code, machine-readable evidence profiles, frozen evidence reports, figure masters, and evidence manifest are maintained in the public DTEP repository (`https://github.com/qiangruhuang/DTEP`). The author-final candidate is based on branch `lc01-algorithm-change-boundary`; `tmsu/evidence/WP1_EVIDENCE_MANIFEST_v1.json` and the experiment-specific frozen reports identify the evidence used for each claim. Before external submission, the final submission commit should be tagged and archived to provide an immutable citation target.

## Figure captions

**Figure 1. Heterogeneous-model unification through the TMSU capability boundary.** The upper trial addresses a stable capability/contract boundary while OpenEaagles and RadarSimPublic retain distinct implementation identities, bindings, and provenance. BP-01 and MS-01 provide bounded evidence for behavior-preserving wrapping and binding-only heterogeneous substitution.

**Figure 2. Evidence ladder and durable uncertainty.** Successive experiments constrain progressively stronger interpretations. The unresolved RF relation remains `UNKNOWN` through later positive results, while the retained VU-01a failure shows that the evidence comparator itself required correction.
...
**Figure 4. Selective carry-forward versus the stopping rule.** VU-01b supports selective carry-forward for a controlled provenance-only revision after a typed comparator and negative control pass; LC-01 rejects automatic inheritance after a substantive model-algorithm/`Implementation_ID` change despite survival of the old execution envelope.
...
| Experiment | Experiment decision | Quantitative anchor | Supported inference | Explicit boundary |
|---|---|---|---|---|
| BP-01 | PASS | 16/16 exact; negative control rejected | Wrapper transparency in the frozen deterministic observation envelope | no universal/hidden-state equivalence |
...
| VU-01a | **FAIL** | exact identity 8/16 | strict byte rule unsuitable for the tested cross-run evidence | not a universal statement about numerical reproducibility |
| VU-01b | PASS | normalized 16/16; `+1e-6 m` control rejected | selective carry-forward for tested provenance revision | not a universal delta-requalification rule |
| LC-01 | PASS (stop-rule test) | CV 16/16; CA 16/16; 12/16 equal; max differences 109.0755 m and 67.6108 m/s | substantive algorithm/identity change crosses the inheritance boundary | discrimination control, not operational validation |
...
`research/SUPPORTING_EVIDENCE_PACKAGE_v1_1.md` organizes detailed methods, figures, and tables for reproducibility. The main manuscript does not depend on acceptance of a separate supplemental file.